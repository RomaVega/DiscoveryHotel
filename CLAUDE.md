...
lib/
  ...
  image-loader.ts       # Custom next/image loader (currently a no-op pass-through)
...
scripts/                # check-images.js, generate-image-manifest.js, optimize-images.js
```
...
- Because `output: "export"` disables Next's built-in optimizer, [lib/image-loader.ts](lib/image-loader.ts) is a pass-through — every image ships at its source resolution. Hand-size files for the largest viewport that will render them.
