            ...
            <div className="divide-y divide-sand">
              {data.routes.map((route, i) => (
                <div key={i} className="grid grid-cols-3 gap-4 p-4">
                  ...
                  <span className="font-sans text-sm text-accent-text text-center font-semibold">{route.oneWay}</span>
                  <span className="font-sans text-sm text-accent-text text-center font-semibold">{route.roundTrip}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="divide-y divide-sand">
                {data.localRoutes.map((route, i) => (
                  <div key={i} className="flex items-center justify-between p-4">
                    <span className="font-sans text-sm text-charcoal">{t(route.destination)}</span>
                    <span className="font-sans text-sm text-accent-text font-semibold">{route.price}</span>
                  </div>
                ))}
              </div>
            </div>
